...
Vue.component("node-ip-address-thresholds-box", {
	...
	<div v-if="isAdding" style="margin-top: 0.5em">
		<table class="ui table celled">
			<thead>
				<tr>
					<th style="background: #f9fafb; box-shadow: none;">阈值</th>
					<th>动作</th>
				</tr>
			</thead>
			<tr>
				<td>
					...
					<div>
						<div v-for="(item, index) in addingThreshold.items" class="ui label basic small" style="margin-bottom: 0.5em;">
							...
							<span v-else>
							 ...
							 &nbsp;
							<a href="" title="删除" @click.prevent="removeItem(index)"><i class="pi pi-times"></i></a>
						</div>
					</div>
					...
					<div v-if="isAddingItem" style="margin-top: 0.8em">
						...
						<div style="margin-top: 0.8em">
							...
							<a href="" title="取消" @click.prevent="cancelItem"><i class="pi pi-times"></i></a>
						</div>
					</div>
				...
				<td>
					...
					<div>
						<div v-for="(action, index) in addingThreshold.actions" class="ui label basic small" style="margin-bottom: 0.5em">
							...
							<span v-if="action.action == 'switch'">到{{action.options.ips.join(", ")}}</span>
							<span v-if="action.action == 'webHook'" class="small grey">({{action.options.url}})</span>
							<a href="" title="删除" @click.prevent="removeAction(index)"><i class="pi pi-times"></i></a>
						</div>
					</div>
					...
					<div v-if="isAddingAction" style="margin-top: 0.8em">
						...
						<div style="margin-top: 0.8em">
							...
							<a href="" title="取消" @click.prevent="cancelAction"><i class="pi pi-times"></i></a>
						</div>
					</div>
		...
		<div>
			...
			<a href="" title="取消" @click.prevent="cancel"><i class="pi pi-times"></i></a>
		</div>
	</div>
...
Vue.component("node-login-suggest-ports", {
	...
	<span v-if="availablePorts.length > 0">
		可能端口：<a href="" v-for="port in availablePorts" @click.prevent="selectPort(port)" class="ui label tiny basic" style="border: 1px #2185d0 dashed; font-weight: normal">{{port}}</a>
		&nbsp; &nbsp;
	</span>
	<span v-if="ports.length > 0">
		常用端口：<a href="" v-for="port in ports" @click.prevent="selectPort(port)" class="ui label tiny basic" style="border: 1px #2185d0 dashed;  font-weight: normal">{{port}}</a>
	</span>
...
Vue.component("node-cache-disk-dirs-box", {
	...
	<div style="margin-bottom: 0.3em">
		<span class="ui label small basic" v-for="(dir, index) in dirs">
			<i class="icon folder"></i>{{dir.path}}  &nbsp;  <a href="" title="删除" @click.prevent="remove(index)"><i class="pi pi-times"></i></a>
		</span>
	</div>
		...
		<tr>
			...
			<td>
				<div v-if="primaryCluster != null">
					<div class="ui label basic small">{{primaryCluster.name}} &nbsp; <a href="" title="删除" @click.prevent="removePrimary"><i class="pi pi-times"></i></a> </div>
				</div>
		...
		<tr>
			...
			<td>
				<div v-if="secondaryClusters.length > 0">
					<div class="ui label basic small" v-for="(cluster, index) in secondaryClusters"><span class="grey">{{cluster.name}}</span> &nbsp; <a href="" title="删除" @click.prevent="removeSecondary(index)"><i class="pi pi-times"></i></a> </div>
				</div>
...
Vue.component("node-ddos-protection-config-box", {
 ...
 <b-notice>功能说明：此功能为<strong>试验性质</strong>，目前仅能防御简单的DDoS攻击，试验期间建议仅在被攻击时启用，仅支持已安装<code-label>nftables v0.9</code-label>以上的Linux系统。<pro-warning-label></pro-warning-label></b-notice>
...
<table class="ui table definition selectable">
	<tbody>
		<tr>
			...
			<td>
				<checkbox v-model="healthCheck.isOn" value="1"></checkbox>
	...
	<tbody v-show="healthCheck.isOn">
		<tr>
			...
			<td>
				...
				<div v-show="urlIsEditing">
					...
					<b-notice v-if="healthCheck.url.length > 0">拼接后的检测URL：<code-label>{{healthCheck.url}}</code-label>，其中\${host}指的是域名。</b-notice>
				</div>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<checkbox v-model="healthCheck.autoDown" value="1"></checkbox>
	...
	template: `
		<span>
			<div class="b-checkbox-container">
				...
				<!-- <input ref="checkbox" type="checkbox" :name="name" :value="elementValue" :id="elementId" @change="change" v-model="newValue"/> -->
				<input ref="checkbox" type="checkbox" :name="name" :value="value" :id="elementId" @change="change" v-model="newValue"/>
			</div>
			<label :for="elementId"><slot></slot></label>
		</span>
	`,
	mounted(){
		this.$refs.checkbox.checked = Boolean(this.newValue);
		this.$emit("input", this.newValue);
		...
		Object.entries(this.$parent.$refs).forEach(([key, value]) => {
			if(Array.isArray(value)){
				value.forEach((v, index) => {
					if(v===this){
						value[index]=this.$refs.checkbox;
					}
				});
			}else{
				if(value===this){
					this.$refs.checkbox[key]=this.$refs.checkbox;
				}
			}
		});
	},
...
Vue.component("bandwidth-size-capacity-box", {
	...
	<div class="ui field">
		<b-select
			...
			@change="change"
			:options="[
				(supportedUnits.length == 0 || supportedUnits.$contains('b')) ? {label: 'Kbps', value: 'kb'} : null,
				(supportedUnits.length == 0 || supportedUnits.$contains('mb')) ? {label: 'Mbps', value: 'mb'} : null,
				(supportedUnits.length == 0 || supportedUnits.$contains('gb')) ? {label: 'Gbps', value: 'gb'} : null,
				(supportedUnits.length == 0 || supportedUnits.$contains('tb')) ? {label: 'Tbps', value: 'tb'} : null,
				(supportedUnits.length == 0 || supportedUnits.$contains('pb')) ? {label: 'Pbps', value: 'pb'} : null,
				(supportedUnits.length == 0 || supportedUnits.$contains('eb')) ? {label: 'Ebps', value: 'eb'} : null,
...
Vue.component("url-patterns-box", {
	...
	<div v-show="isAdding" style="margin-top: 0.5em">
		<div :class="{'ui fields inline': !windowIsSmall}">
			<div class="ui field">
				<b-select
					v-model="addingPattern.type"
					auto-width
					:options="[
						{label: '通配符', value: 'wildcard'},
						{label: '正则表达式', value: 'regexp'},
						{label: '常见图片', value: 'images'},
						{label: '常见音频', value: 'audios'},
						{label: '常见视频', value: 'videos'},
					]"
				></b-select>
			...
			<div class="ui field">
				<button class="ui button tiny" :class="{disabled:this.patternIsInvalid}" type="button" @click.prevent="confirm">确定</button><a href="" title="取消" @click.prevent="cancel"><i class="pi pi-times"></i></a>
			</div>
		</div>
	</div>
	<div v-if=!isAdding style="margin-top: 0.5em">
...
Vue.component("pro-warning-label", {
	template: `<span><i class="p-message-icon pi pi-exclamation-triangle yellow"></i>注意：通常不需要修改；如要修改，请在专家指导下进行。</span>`
})
...
Vue.component("combo-box", {
	...
	template: `
		<div style="display: inline; z-index: 10; background: white" class="combo-box">
			<!-- 搜索框 -->
			...
			<div v-if="selectedItem != null">
				...
				<span class="ui label basic" style="line-height: 1.4; font-weight: normal; font-size: 1em" ref="selectedLabel"><span><span v-if="title != null && title.length > 0">{{title}}：</span>{{selectedItem.name}}</span>
					<a href="" title="清除" @click.prevent="reset"><i class="pi pi-times"></i></a>
				</span>
			</div>
...
Vue.component("search-box", {
	...
	<div class="ui input small" :class="{'right labeled': realValue.length > 0}">
		...
		<a href="" class="ui label" v-if="realValue.length > 0" @click.prevent="clearValue" style="padding-right: 0"><i class="icon remove"></i></a>
	</div>
</div>`
})
	...
	<div v-show="isEditing || realValues.length == 0">
		...
		<div v-if="isAdding || isUpdating">
			<div class="ui fields inline">
				...
				<div class="ui field">
					<a href="" @click.prevent="cancel()" title="取消"><i class="pi pi-times"></i></a> 
				</div> 
			</div> 
		</div> 
		<div v-if="!isAdding && !isUpdating">
			<b-add-button @click="create()"></b-add-button>
		</div>
	</div>	
</div>`
});
...
Vue.component("ip-list-bind-box", {
	...
	<div class="ui label basic small" v-for="(list, index) in lists">
		...
		<a href="" title="删除" @click.prevent="remove(index, list.id)"><i class="pi pi-times"></i></a>
	</div>
</div>`
})
...
Vue.component("ip-list-table", {
 ...
 <table class="ui table selectable celled" v-if="items.length > 0">
        <thead>
            <tr>
            	<th style="width: 1em">
								<checkbox v-model="selectedAll" @change="changeSelectedAll" />
							</th>
		...
		<tbody v-for="item in items">
			<tr>
				<td>
					<checkbox :value="item.id" @input="changeSelected" ref="itemCheckBox"></checkbox>
...
Vue.component("origin-list-box", {
	...
	<b-empty v-if="primaryOrigins.length == 0">暂时还没有主要源站。</b-empty>
	...
	<b-empty v-if="backupOrigins.length == 0">暂时还没有备用源站。</b-empty>
	...
	<table class="ui table definition selectable">
		...
		<tbody v-show="(!vIsLocation && !vIsGroup) || config.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="config.isOn" value="1"></checkbox>
			...
			<tr>
				...
				<td>
					...
					<div v-if="isAdding" style="margin-top: 0.5em">
						<table class="ui table definition">
							...
							<tr>
								...
								<td>
									...
									<!--<select class="ui dropdown auto-width" v-model="addingFilter.action">
						...
						<button type="button" class="ui button tiny" @click.prevent="confirm">保存</button> &nbsp; <a href="" @click.prevent="cancel" title="取消"><i class="pi pi-times"></i></a>
					</div>
					<div v-show="!isAdding" style="margin-top: 0.5em">
						<b-add-button @click="add"></b-add-button>
					</div>
...
Vue.component("http-common-config-box", {
	...
	<table class="ui table definition selectable">
		<tr>
			...
			<td>
				<checkbox v-model="config.mergeSlashes" name="mergeSlashes" value="1"></checkbox>
...
<table class="ui table selectable definition">
	...
	<tbody v-show="!vIsLocation || vGzipRef.isPrior">
		<tr>
			...
			<td>
				<checkbox v-model="vGzipRef.isOn"></checkbox>
			</td>
		</tr>
	</tbody>
	<tbody v-show="isOn()">
...
Vue.component("http-cond-url-extension", {
	...
	<div v-if="extensions.length > 0">
		<div class="ui label small basic" v-for="(ext, index) in extensions">{{ext}} <a href="" title="删除" @click.prevent="removeExt(index)"><i class="pi pi-times"></i></a></div>
		<div class="ui divider"></div>
	</div>
...
Vue.component("http-cond-params", {
	...
	<tr v-if="['regexp', 'not regexp', 'eq', 'not', 'prefix', 'suffix', 'contains', 'not contains', 'in', 'not in'].$contains(cond.operator)">
		<td>不区分大小写</td>
		<td>
			<checkbox name="condIsCaseInsensitive" v-model="cond.isCaseInsensitive"></checkbox>
...
Vue.component("http-access-log-box", {
	...
	<div>
		<a v-if="accessLog.node != null && accessLog.node.nodeCluster != null" :href="'/clusters/cluster/node?nodeId=' + accessLog.node.id + '&clusterId=' + accessLog.node.nodeCluster.id" title="点击查看节点详情" target="_top">
			<span class="grey">
				[{{accessLog.node.name}}<span v-if="!accessLog.node.name.endsWith('节点')">节点</span>]
			</span>
		</a>
	...
	<thead>
		<tr>
			...
			this.stats.forEach(function (v) {
				...
				table += "<td><div class=\"ui progress\"><div class=\"bar\" style=\"min-width: 0; height: 4px; width: " + percent + "%\"></div></div>" + percent + "%</td>"
				table += "</tr>"
			})
	...
	<table class="ui table selectable definition" v-if="vIsLocation">
		...
		<tbody v-show="redirectToHttpsConfig.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="redirectToHttpsConfig.isOn"></checkbox>
					...
					<table class="ui table" v-show="moreOptionsVisible">
						<tr>
							...
							<td>
								<b-select
									v-model="redirectToHttpsConfig.status"
									auto-width
									:options="[
										{label: '[使用默认]', value: '0'},
    ...
    <table class="ui table selectable definition">
      ...
      <tbody v-show="moreOptionsVisible">
        <tr>
          ...
          <td>
            <b-select
              v-model="firewall.defaultCaptchaType"
              auto-width
              :options="[
                {label: '默认', value: 'none'},
                ...captchaTypes
									.filter(captchaType=>captchaType.code != 'geetest' || geeTestIsOn)
									.map(captchaType => ({
										label: captchaType.name??'',
										value: captchaType.code??'',
									})),
              ]"
            ></b-select>
	...
	<table class="ui table definition selectable">
		<tbody>
			<tr v-show="vProtocol == 'https'">
				...
				<td>
					<checkbox v-model="policy.http2Enabled" value="1"></checkbox>
				</td>
			</tr>
			<tr v-show="vProtocol == 'https' && vSupportHttp3">
				<td class="title">启用HTTP/3</td>
				<td>
					<checkbox v-model="policy.http3Enabled" value="1"></checkbox>
				</td>
			</tr>
			<tr>
		...
		<tbody v-show="moreOptionsVisible">
			...
			<tr>
				...
				<td>
					<checkbox v-model="policy.cipherSuitesIsOn" value="1"></checkbox>
			...
			<tr v-show="vProtocol == 'https'">
				...
				<td>
					<checkbox v-model="hsts.isOn" value="1"></checkbox>
					<p class="comment">
						 开启后，会自动在响应Header中加入
			...
			<tr v-show="hsts.isOn && hstsOptionsVisible">
				...
				<td>
					<checkbox v-model="hsts.includeSubDomains" name="hstsIncludeSubDomains" value="1"></checkbox>
				</td>
			</tr>
			<tr v-show="hsts.isOn && hstsOptionsVisible">
				<td class="color-border">HSTS预加载<em>（preload）</em></td>
				<td>
					<checkbox v-model="hsts.preload" name="hstsPreload" value="1"></checkbox>
				</td>
			</tr>
			<tr v-show="hsts.isOn && hstsOptionsVisible">
				...
				<td colspan="2">
					...
					<div class="ui field" style="margin-top: 1em">
						<b-add-button @click="addHstsDomain()"></b-add-button>
					</div>
			...
			<tr>
				...
				<td>
					<b-select
						name="clientAuthType"
						v-model="policy.clientAuthType"
						auto-width
						:options="[
							...allClientAuthTypes.map(authType => ({
								label: authType.name??'',
								value: authType.type??'',
							}))
						]"
...
<table class="ui table selectable definition">
	...
	<tbody v-show="!vIsLocation || authConfig.isPrior">
		<tr>
			...
			<td>
				<checkbox v-model="authConfig.isOn"></checkbox>
			</td>
		</tr>
	</tbody>
</table>
	...
	<table class="ui table definition selectable">
		...
		<tbody v-show="(!vIsLocation && !vIsGroup) || stat.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="stat.isOn"></checkbox>
				</td>
			</tr>
		</tbody>
	</table>
...
Vue.component("http-header-policy-box", {
	...
	<div v-if="((!vIsLocation && !vIsGroup) || requestHeaderRef.isPrior) && type == 'request'">
		...
		<h4>设置请求报头 &nbsp; <a href="" @click.prevent="addSettingHeader(vRequestHeaderPolicy.id)" style="font-size: 0.8em">[添加新报头]</a></h4>
			<b-empty v-if="requestSettingHeaders.length == 0">暂时还没有自定义报头。</b-empty>
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<b-add-button @click="addDeletingHeader(vRequestHeaderPolicy.id, 'request')"></b-add-button>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<b-add-button @click="addNonStandardHeader(vRequestHeaderPolicy.id, 'request')"></b-add-button>
						</td>
					</tr>
				</tbody>
			</table>
	...
	<div v-if="((!vIsLocation && !vIsGroup) || responseHeaderRef.isPrior) && type == 'response'">
    	...
    	<div :class="{'opacity-mask': vHasGroupResponseConfig}">
			...
			<b-empty v-if="responseSettingHeaders.length == 0">暂时还没有自定义报头。</b-empty>
					...
					<tr>
						...
						<td>
							<div v-if="responseDeletingHeaders.length > 0">
								<div class="ui label small basic" v-for="headerName in responseDeletingHeaders">{{headerName}} &nbsp; <a href=""><i class="pi pi-times" title="删除" @click.prevent="deleteDeletingHeader(vResponseHeaderPolicy.id, headerName)"></i></a></div>
								<div class="ui divider" ></div>
							</div>
							<b-add-button @click="addDeletingHeader(vResponseHeaderPolicy.id, 'response')"></b-add-button>
						</td>
					</tr>
					<tr>
						...
						<td>
							<div v-if="responseNonStandardHeaders.length > 0">
								<div class="ui label small basic" v-for="headerName in responseNonStandardHeaders">{{headerName}} &nbsp; <a href=""><i class="pi pi-times" title="删除" @click.prevent="deleteNonStandardHeader(vResponseHeaderPolicy.id, headerName)"></i></a></div>
								<div class="ui divider" ></div>
							</div>
							<b-add-button @click="addNonStandardHeader(vResponseHeaderPolicy.id, 'response')"></b-add-button>
						</td>
					</tr>
					<tr>
...
Vue.component("http-firewall-policy-selector", {
	...
	<div v-if="firewallPolicy != null" class="ui label basic">
		...
		{{firewallPolicy.name}} &nbsp; <a :href="'/servers/components/waf/policy?firewallPolicyId=' + firewallPolicy.id" target="_blank" title="修改"><i class="icon pencil small"></i></a>&nbsp; <a href="" @click.prevent="remove()" title="删除"><i class="pi pi-times"></i></a>
	</div>
	<div v-if="firewallPolicy == null">
		<span v-if="count > 0">
			<a href="" @click.prevent="select">[选择已有策略]</a> &nbsp; &nbsp;
		</span>
...
Vue.component("http-cache-policy-selector", {
	...
	<div v-if="cachePolicy != null" class="ui label basic">
		...
		{{cachePolicy.name}} &nbsp; <a :href="'/servers/components/cache/update?cachePolicyId=' + cachePolicy.id" target="_blank" title="修改"><i class="icon pencil small"></i></a>&nbsp; <a href="" @click.prevent="remove()" title="删除"><i class="pi pi-times"></i></a>
	</div>
	...
	<table class="ui table selectable definition">
		...
		<tbody v-show="!vIsLocation || shutdownConfig.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="shutdownConfig.isOn" value="1"></checkbox>
		...
		<tbody v-show="(!vIsLocation || shutdownConfig.isPrior) && shutdownConfig.isOn">
			<tr>
				...
				<td>
					<b-select
						v-model="shutdownConfig.bodyType"
						auto-width
						:options="[
							{label: 'HTML', value: 'html'},
							{label: '读取URL', value: 'url'},
							{label: '跳转URL', value: 'redirectURL'},
						]"
					></b-select>
	...
	<table class="ui table definition selectable" :class="{'opacity-mask': this.accessLog.firewallOnly}">
		...
		<tbody v-show="(!vIsLocation && !vIsGroup) || accessLog.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="accessLog.isOn"></checkbox>
				</td>
			</tr>
		</tbody>
		...
		<tbody v-show="((!vIsLocation && !vIsGroup) || accessLog.isPrior) && accessLog.isOn && showAdvancedOptions">
			...
			<tr>
				<td>高级信息</td>
				<td>
					<checkbox v-for="(field, index) in vFields" style="width:10em;margin-bottom:0.8em" v-model="field.isChecked" @change="changeFields">
						{{field.name}}
					</checkbox>
			...
			<tr>
				<td>要存储的访问日志状态码</td>
				<td>
					<checkbox v-model="accessLog.status1" style="width:3.5em">1xx</checkbox>
					<checkbox v-model="accessLog.status2" style="width:3.5em">2xx</checkbox>
					<checkbox v-model="accessLog.status3" style="width:3.5em">3xx</checkbox>
					<checkbox v-model="accessLog.status4" style="width:3.5em">4xx</checkbox>
					<checkbox v-model="accessLog.status5" style="width:3.5em">5xx</checkbox>
				</td>
			</tr>
			<tr>
				<td>记录客户端中断日志</td>
				<td>
					<checkbox v-model="accessLog.enableClientClosed"></checkbox>
	...
	<table class="ui table definition selectable">
		...
		<tbody v-show="(!vIsLocation && !vIsGroup) || cacheConfig.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="cacheConfig.isOn"></checkbox>
				</td>
			</tr>
		</tbody>
							...
							<tr v-show="cacheConfig.key.isOn">
								...
								<td>
									<div class="ui fields inline">
										<div class="ui field">
											<b-select
												v-model="cacheConfig.key.scheme"
												:options="[
													{label: 'https://', value: 'https'},
													{label: 'http://', value: 'http'},
												]"
	...
	<div v-show="isOn()" style="margin-top: 1em">
		<h4 style="position: relative">缓存条件 &nbsp; <a href="" style="font-size: 0.8em" @click.prevent="$refs.cacheRefsConfigBoxRef.addRef(false)">[添加]</a> &nbsp; <a href="" style="font-size: 0.8em" @click.prevent="showSearchBox" v-show="!searchBoxVisible">[搜索]</a> 
			<div class="ui input small right labeled" style="position: absolute; top: -0.4em; margin-left: 0.5em; zoom: 0.9" v-show="searchBoxVisible">
				...
				<a href="" class="ui label" @click.prevent="showSearchBox"><i class="pi pi-times"></i></a>
			</div>
		</h4>
	...
	<table class="ui table definition selectable">
		...
		<tbody v-show="(!this.vIsLocation || this.fastcgiRef.isPrior)">
			<tr>
				...
				<td>
					<checkbox v-model="fastcgiRef.isOn"></checkbox>
				</td>
			</tr>
		</tbody>
		<tbody v-if="isOn()">
...
Vue.component("http-rewrite-rule-list", {
	...
	template: `<div>
	<div class="margin"></div>
	<b-empty v-if="rewriteRules.length == 0">暂时还没有重写规则。</b-empty>
...
<address>Request ID: \${requestId}</address>
	...
	<div v-show="configs.length > 0" style="margin-bottom: 0.5em" id="actions-box"> 
		<div v-for="(config, index) in configs" :data-index="index" :key="config.id" class="ui label small basic" :class="{blue: index == editingIndex}" style="margin-bottom: 0.4em">
			...
			<!-- 操作按钮 -->
			 &nbsp; <a href="" title="修改" @click.prevent="update(index, config)"><i class="icon pencil small"></i></a> &nbsp; <a href="" title="删除" @click.prevent="remove(index)"><i class="pi pi-times"></i></a> &nbsp; <a href="" title="拖动改变顺序"><i class="icon bars handle"></i></a>
		</div>
		<div class="ui divider"></div>
	</div>
				...
				<td>
					<b-select
						...
						:options="[
							...
							{label: '网站', value: 'server'},
							{label: '网站和策略', value: 'global'},
						]"
					></b-select>
			...
			<tr v-if="actionCode == 'block'">
				...
				<td>
					<b-select
						...
						auto-width
						:options="[
							{label: '当前网站', value: 'service'},
							{label: '所有网站', value: 'global'},
						]"
					></b-select>
...
Vue.component("defense-config-box", {
	...
	data: function () {
		...
		// riskTags.forEach(function (v) {
		// 	v.isChecked = config.riskTags.$contains(v.id)
		// })
		let riskLevels = ["未选中","低风险","中风险","高风险"]
		let riskActions = [{id:1, name:"5秒盾"},{id:2, name:"无感验证"},{id:3, name:"验证码"},{id:4, name:"封禁"}]
		return {
			isAdding: false,
			addRecord: {
				enable: true,
...
<table class="ui table definition selectable">
	<tbody>
		<tr>
			<td class="title">启用主动防御</td>
			<td>
				<checkbox v-model="config.isOn"></checkbox>
				<p class="comment"><plus-label></plus-label>启用后，基于风险IP预先识别和拦截攻击。</p>
			</td>
		</tr>						
	</tbody>
	<tbody>
			<tr>
				<td>规则列表</td>
				<td>
					<div v-if="config.records && config.records.length > 0">
						<table class="ui table definition">
						 <thead class="full-width">
							<tr>
								<th>风险标签</th>
						...
						<tbody>
							<tr v-if="config.records.length > 0" v-for="(record,idx) in config.records" :key="idx">
								<td>
									<div v-for="tag in riskTags" :key="tag.id" v-if="record.riskTags.$contains(tag.id)">
										{{tag.name}}
									</div>	
								</td>
								<td>{{riskLevels[record.riskLevel]}}</td>
								<td>{{record.riskScore||0}}</td>
								<td>{{riskActions[record.action-1].name}}</td>
								<td>{{record.recordOnly ? "是" : "否"}}</td>
								<td>
									<a href="" @click.prevent="remove(idx)" title="删除"><i class="icon trash small"></i></a>
							</tr>
						</tbody>
						</table>
					</div>
					<div v-if="isAdding" style="margin-top: 0.5em">
						<table class="ui table definition">
							 <tr>
								...
								<td>
									<div>
										<div style="width: 8em; float: left; margin-bottom: 1em" v-for="tag in riskTags" :key="tag.id">
											<checkbox v-model="tag.isChecked" @input="changeRiskTag">{{tag.name}}</checkbox>
										</div>
										<div style="clear: both"></div>
									</div>
									<p class="comment">主动防御将会匹配已选择的风险标签。</p>
								</td> 
							</tr>
							<tr>
								<td>风险等级</td>
								<td>
									...
									<!--<select class="ui dropdown auto-width" v-model="addRecord.riskLevel">
										<option v-for="(level,idx) in riskLevels"  :value="idx">{{level}}</option>
									</select>-->
									<p class="comment">主动防御将会记录或拦截包含该等级及更高等级的风险，建议风险等级和风险评分选择启用一项</p>
								</td>
							</tr>
							<tr>
								<td>风险评分</td>
								<td>
									<div class="ui input right labeled">
										...
										<span class="ui label">取值区间[0-100]</span>
									</div>
									<p class="comment">分值越低，风险越小。为0则表示主动防御不将此参数纳入风险计算条件。</p>
								</td>
							</tr>	
							<tr>
								<td>处理动作</td>
								<td>
									<div v-for="act in riskActions" class="ui radio checkbox">
    <input type="radio" :id="'action-' + act.id" v-model="addRecord.action" :value="act.id">
    <label :for="'action-' + act.id">{{act.name}}</label>
</div>
									<p class="comment">对于触发以上条件的请求，选择对应的处理动作。</p>
								</td>
							</tr>
							<tr>
								<td>观察模式</td>
								<td>
									<checkbox v-model="addRecord.recordOnly"></checkbox>
									<p class="comment">启用后，处理动作不会生效，只记录风险行为。</p>
								</td>
							</tr>
						</table>
						<button type="button" class="ui button tiny" @click.prevent="save">保存</button> &nbsp; <a href="" @click.prevent="cancel" title="取消"><i class="pi pi-times"></i></a>
					</div>
					<div v-show="!isAdding" style="margin-top: 0.5em">
						<b-add-button @click="add"></b-add-button>
					</div>
...
Vue.component("http-header-replace-values", {
	...
	<div>
		<div v-for="(value, index) in values" class="ui label small" style="margin-bottom: 0.5em">
			...
			<a href="" @click.prevent="remove(index)" title="删除"><i class="pi pi-times"></i></a>
		</div>
	</div>
	<div v-if="isAdding">
		<table class="ui table">
		...
		<table class="ui table definition selectable">
			<tbody>
				<tr>
					...
					<td>
						<checkbox v-model="config.html.isOn" value="1"></checkbox>
		...
		<table class="ui table definition selectable">
			<tbody>
				<tr>
					...
					<td>
						<checkbox v-model="config.javascript.isOn" value="1"></checkbox>
		...
		<table class="ui table definition selectable">
			<tbody>
				<tr>
					...
					<td>
						<checkbox v-model="config.css.isOn" value="1"></checkbox>
...
Vue.component("ssl-certs-view", {
	...
	<div v-if="certs != null && certs.length > 0">
		<div class="ui label small basic" v-for="(cert, index) in certs">
			{{cert.name}} / {{cert.dnsNames}} / 有效至{{formatTime(cert.timeEndAt)}} &nbsp;<a href="" title="查看" @click.prevent="viewCert(cert.id)"><i class="icon expand"></i></a>
		</div>
	</div>
</div>`
})
	...
	<table class="ui table definition selectable">
		...
		<tbody v-show="((!vIsLocation && !vIsGroup) || websocketRef.isPrior)">
			<tr>
				...
				<td>
					<checkbox v-model="websocketRef.isOn"></checkbox>
				</td>
			</tr>
		</tbody>
		<tbody v-show="isOn()">
			<tr>
				...
				<td>
					<checkbox v-model="websocketConfig.allowAllOrigins"></checkbox>
				...
				<td>
					<div v-if="websocketConfig.allowedOrigins.length > 0">
						<div class="ui label small basic" v-for="(origin, index) in websocketConfig.allowedOrigins">
							{{origin}} <a href="" title="删除" @click.prevent="removeOrigin(index)"><i class="pi pi-times"></i></a>
						</div>
						<div class="ui divider"></div>
					</div>
					<b-add-button @click="createOrigin()"></b-add-button>
		...
		<tbody v-show="isOn() && advancedVisible">
			<tr>
				...
				<td>
					<checkbox v-model="websocketConfig.requestSameOrigin"></checkbox>
		...
		<tbody v-show="isOn() && advancedVisible">
			<tr>
				...
				<td>
					...
					<b-add-button @click="addIndex()"></b-add-button>
			...
			<tr>
				<td>路径解码</td>
				<td>
					<checkbox v-model="config.decodePath"></checkbox>
			...
			<tr>
				<td>终止请求</td>
				<td>
					<checkbox v-model="config.isBreak"></checkbox>
...
Vue.component("http-host-redirect-box", {
	...
	<b-empty v-if="redirects.length == 0">暂时还没有URL跳转规则。</b-empty>
...
Vue.component("http-cache-ref-box", {
	...
	<tr v-if="condCategory == 'simple'">
		...
		<td>
			<b-select
				...
				auto-width
				:options="[
					{label: '文件扩展名', value: 'url-extension'},
					{label: '首页', value: 'url-eq-index'},
					{label: '全站', value: 'url-all'},
					{label: 'URL目录前缀', value: 'url-prefix'},
					{label: 'URL完整路径', value: 'url-eq'},
					{label: 'URL通配符', value: 'url-wildcard-match'},
					{label: 'URL正则匹配', value: 'url-regexp'},
					{label: '参数匹配', value: 'params'},
				]"
			></b-select>
	...
	<tr v-if="condCategory == 'simple' && condComponent.caseInsensitive">
		<td>不区分大小写</td>
		<td>
			<checkbox v-model="condIsCaseInsensitive" value="1" name="condIsCaseInsensitive"></checkbox>
			<p class="comment">选中后表示对比时忽略参数值的大小写。</p>
		</td>
	</tr>
	...
	<tr v-show="moreOptionsVisible && !vIsReverse">
		<td>跳过Set-Cookie</td>
		<td>
			<checkbox value="1" v-model="ref.skipSetCookie"></checkbox>
	...
	<tr v-show="moreOptionsVisible && !vIsReverse">
		...
		<td>
			<checkbox v-model="ref.enableRequestCachePragma" name="enableRequestCachePragma" value="1"></checkbox>
...
Vue.component("metric-keys-config-box", {
	...
	<div>
		<div v-for="(key, index) in keys" class="ui label small basic">
			{{keyName(key)}} &nbsp; <a href="" title="删除" @click.prevent="remove(index)"><i class="pi pi-times"></i></a>
		</div>
	</div>
	<div v-if="isAdding" style="margin-top: 1em">
		<div class="ui fields inline">
	...
	<table class="ui table selectable definition">
		...
		<tbody v-show="(!vIsLocation && !vIsGroup) || reverseProxyRef.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="reverseProxyRef.isOn"></checkbox>
		...
		<tbody v-show="isOn() && advancedVisible">
			...
			<tr v-if="family == null || family == 'http'">
				...
				<td>
					<checkbox v-model="reverseProxyConfig.autoFlush"></checkbox>
			...
			<tr v-show="family != 'unix' && reverseProxyConfig.proxyProtocol.isOn">
				...
				<td>
					<b-select
						...
						v-model="reverseProxyConfig.proxyProtocol.version"
						auto-width
						:options="[
							{label: '1', value: '1'},
							{label: '2', value: '2'},
	...
	<table class="ui table definition selectable">
		...
		<tbody v-show="(!vIsLocation && !vIsGroup) || charsetConfig.isPrior">
			<tr>
				...
				<td>
					<checkbox v-model="charsetConfig.isOn"></checkbox>
				</td>
			</tr>
		</tbody>
			...
			<tr>
				...
				<td>
					<b-select
						style="width:20em"
						name="charset"
						v-model="charsetConfig.charset"
						:options="[
							{label: '[未选择]', value: ''},

							{label: '常用字符编码', type: 'header'},
							...vUsualCharsets.map(charset => ({
								label: (charset.charset??'') + '（' + (charset.name??'') + '）',
								value: charset.charset??'',
							})),

							{label: '全部字符编码', type: 'header'},
							...vAllCharsets.map(charset => ({
								label: (charset.charset)??'' + '（' + (charset.name??'') + '）',
								value: charset.charset??'',
							})),
						]"
					...
					<!--<select class="ui dropdown" style="width:20em" name="charset" v-model="charsetConfig.charset">
		...
		<tbody v-show="((!vIsLocation && !vIsGroup) || charsetConfig.isPrior) && charsetConfig.isOn && advancedVisible">
			...
			<tr>
				<td>字符编码大写</td>
				<td>
					<checkbox v-model="charsetConfig.isUpper"></checkbox>
	...
	<table class="ui table definition selectable">
		...
		<tbody v-show="isOn() && moreOptionsVisible">
			<tr>
				<td>压缩算法</td>
				<td>
					<checkbox v-model="config.useDefaultTypes">
						<template v-if="config.useDefaultTypes">
							使用默认顺序<span class="grey small">（brotli、gzip、 zstd、deflate）</span>
						</template>
						<template v-else>
							使用默认顺序
						</template>
					...
					<div v-show="!config.useDefaultTypes">
						...
						<div id="compression-types-box">
							<template v-for="t in allTypes">
								<checkbox style="margin-right: 2em" :data-code="t.code" v-model="t.isOn" @change="changeType">
									{{t.name}} &nbsp; <i class="icon list small grey handle"></i>
								</checkbox>
							</template>
						</div>
					</div>
...
Vue.component("http-cache-refs-config-box", {
	...
	<div>
		<b-empty v-if="refs.length == 0">暂时还没有缓存条件。</b-empty>
			...
			<tr>
				...
				<td>
					<b-select
						...
						auto-width
						:options="[
							{label: '自定义HTTP接口', value: 'webHook'},
							{label: '阿里云短信', value: 'aliyunSMS'},
							{label: '腾讯云短信', value: 'tencentSMS'},
						]"
					></b-select>
		...
		<tr>
			...
			<td>
				<b-select
					...
					auto-width
					:options="[
						{label: '峰值带宽', value: 'secondly'},
						{label: '平均带宽', value: 'avg'},
					]"
				></b-select>
...
Vue.component("plan-bandwidth-ranges", {
	...
	<div v-if="isAdding">
		...
		<button class="ui button small" type="button" @click.prevent="confirm">确定</button> &nbsp;
		<a href="" title="取消" @click.prevent="cancelAdding"><i class="pi pi-times"></i></a>
	</div>
...
Vue.component("node-init-script-selector", {
	props: ["v-script", "v-script-id"],
	data: function () {
		return {
			scriptId: (this.vScript == null) ? 0 : this.vScript.id,
			script: this.vScript,
		}
	},
	methods: {
		...
		select: function () {
			let that = this;
			teaweb.popup("/clusters/grants/scripts/selectPopup", {
				title: '选择节点初始化脚本',
				callback: (resp) => {
					that.scriptId = resp.data.script.id;
					if (that.scriptId > 0) {
						that.script = resp.data.script;
					}
					that.notifyUpdate()
				},
				height: "26em"
		...
		create: function () {
			let that = this
			teaweb.popup("/clusters/grants/scripts/createPopup", {
				title: '创建脚本',
				height: "26em",
				callback: (resp) => {
					that.scriptId = resp.data.script.id;
		...
		update: function () {
			...
			let that = this
			teaweb.popup("/clusters/grants/scripts/updatePopup?scriptId=" + this.script.id, {
				title: '修改脚本',
				height: "26em",
				callback: (resp) => {
					that.notifyUpdate()
	...
	<div class="ui label small basic" v-if="script != null">{{script.name}} <a href="" title="修改" @click.prevent="update()"><i class="icon pencil small"></i></a> <a href="" title="删除" @click.prevent="remove()"><i class="icon remove"></i></a> </div>
	<div v-if="script == null">
		<a href="" @click.prevent="select()">[选择已有脚本代码]</a> &nbsp; &nbsp; <a href="" @click.prevent="create()">[添加新脚本代码]</a>
	</div>
</div>`
})
...
Vue.component("ad-instance-objects-box", {
	...
	<!-- 已有对象 -->
	<div>
		<b-empty v-if="objects.length == 0">暂时还没有设置任何防护对象。</b-empty>
		<div v-if="objects.length > 0">
			<table class="ui table">
				<tr>
					<td class="title">已选中防护对象</td>
					<td>
						<b-label-group style="margin-bottom: 0.5em">
							<b-label v-for="(object, index) in objects" size="small">
...
Vue.component("report-node-groups-selector", {
	...
	<div v-if="groups.length > 0">
		<div>
			<checkbox v-model="allGroups">全部分组</checkbox>
		...
		<div v-show="!allGroups">
			<div v-for="group in groups" :key="group.id" style="float: left; width: 7.6em; margin-bottom: 0.5em">
				<checkbox v-model="group.isChecked" value="1" @click.prevent="check(group)">
					{{group.name}}
				</checkbox>
			</div>
		</div>
...
Vue.component("dns-route-selector", {
	...
	<b-add-button @click="add" v-if="!isAdding"></b-add-button>
	<div v-if="isAdding">
		<table class="ui table">
			<tr>
				...
				<td>
					...
					<span v-show="keyword.length == 0 || searchingRoutes.length > 0">
						<b-select
							v-model="routeCode"
							:options="[
								(keyword.length == 0) ? {label: '[请选择]', value: ''} : null,
								...searchingRoutes.map(route => ({
									label: (route.name??'') + '（' + (route.code??'') + '/' + (route.domainName??'') + '）',
									value: (route.code??'') + '@' + (route.domainId??''),
								}))
							].filter(v => v)"
						></b-select>
			...
			<tr>
				...
				<td>
					<div class="ui input" :class="{'right labeled':keyword.length > 0}">
						...
						<a class="ui label" v-if="keyword.length > 0" @click.prevent="clearKeyword" href=""><i class="pi pi-times"></i></a>
					</div>
				</td>
			</tr>
		</table>
		
		<button class="ui button tiny" type="button" @click.prevent="confirm">确定</button> &nbsp; <a href="" @click.prevent="cancel()"><i class="pi pi-times"></i></a>
	</div>
</div>`
})
	...
	<table class="ui table definition selectable">
		<tbody>
			<tr>
				<td class="title">启用</td>
				<td>
					<checkbox v-model="config.isOn" name="isOn" value="1"></checkbox>
		...
		<tbody v-show="config.isOn">
			<tr>
				...
				<td>
					<checkbox v-model="config.useLocalHosts" name="useLocalHosts" value="1"></checkbox>
				...
				<td>
					<div v-if="config.hosts.length > 0">
						<div v-for="(host, index) in config.hosts" class="ui label tiny basic">
							...
							<a href="" title="删除" @click.prevent="removeHost(index)"><i class="pi pi-times"></i></a>
						</div>
						<div class="ui divider"></div>
					</div>
					<div v-if="hostIsAdding">
						<div class="ui fields inline">
							...
							<div class="ui field">
								<button class="ui button tiny" type="button" @click.prevent="confirmHost">确认</button> &nbsp; <a href="" title="取消" @click.prevent="cancelHost"><i class="pi pi-times"></i></a>
							</div>
						</div>
					</div>
					<div style="margin-top: 0.5em">
						<b-add-button @click="addHost"></b-add-button>
					</div>
				</td>
			</tr>
			<tr>
...
Vue.component("ns-routes-selector", {
	...
	<div v-show="selectedRoutes.length > 0">
		<div class="ui label basic text small" v-for="(route, index) in selectedRoutes" style="margin-bottom: 0.3em">
			...
			{{route.name}} &nbsp; <a href="" title="删除" @click.prevent="remove(index)"><i class="pi pi-times"></i></a>
		</div>
		<div class="ui divider"></div>
	</div>
	<div v-if="isAdding" style="margin-bottom: 1em">
		<table class="ui table">
				...
				<td>
					<b-select
						...
						:options="[
							...
							{label: '自定义线路', value: 'user'},
							{label: '运营商', value: 'isp'},
							{label: '中国省市', value: 'china'},
							{label: '全球国家地区', value: 'world'},
							{label: '搜索引擎', value: 'agent'},
						]"
					></b-select>
			...
			<tr>
				...
				<td>
					<b-select
						...
						auto-width
						:options="[
							...routes.filter(route=>route.type === routeType).map(route => ({
								label: route.name??'',
								value: route.code??'',
							}))
						]"
			...
			<tr v-if="routeCode.length > 0 && provinces[routeCode] != null">
				...
				<td>
					<b-select
						v-model="provinceRouteCode"
						auto-width
						:options="[
							{label: '[全域]', value: ''},
							...provinces[routeCode].map(province => ({
								label: province.name??'',
								value: province.code??'',
							}))
						]"
	...
	<div v-if="rangeType == 'ipRange'">
		...
		<div style="margin-bottom: 1em" v-show="isAdding">
			...
			<button class="ui button tiny" type="button" @click.prevent="confirmIPRange">确定</button> &nbsp;
					<a href="" @click.prevent="cancelIPRange" title="取消"><i class="pi pi-times"></i></a>
		</div>
		...
		<div style="margin-bottom: 1em" v-show="isAddingBatch">
			...
			<button class="ui button tiny" type="button" @click.prevent="confirmBatchIPRange">确定</button> &nbsp;
				<a href="" @click.prevent="cancelBatchIPRange" title="取消"><i class="pi pi-times"></i></a>
		</div>
	...
	<div v-if="rangeType == 'cidr'">
		...
		<div style="margin-bottom: 1em" v-show="isAdding">
			...
			<button class="ui button tiny" type="button" @click.prevent="confirmIPCIDR">确定</button> &nbsp;
					<a href="" @click.prevent="cancelIPCIDR" title="取消"><i class="pi pi-times"></i></a>
		</div>
		...
		<div style="margin-bottom: 1em" v-show="isAddingBatch">
			...
			<button class="ui button tiny" type="button" @click.prevent="confirmBatchIPCIDR">确定</button> &nbsp;
				<a href="" @click.prevent="cancelBatchIPCIDR" title="取消"><i class="pi pi-times"></i></a>
		</div>
	...
	<div v-if="rangeType == 'region'">
		...
		<div v-if="isAdding">
					...
					<td>
						<span v-for="(region, index) in regions">
							<span class="ui label small basic">
								...
								<span v-if="region.type == 'city'">城市</span>
								<span v-if="region.type == 'provider'">ISP</span>
								：{{region.name}} <a href="" title="删除" @click.prevent="removeRegion(index)"><i class="pi pi-times"></i></a>
							</span>
			...
			<button class="ui button tiny" type="button" @click.prevent="confirmRegions">确定</button> &nbsp;
				<a href="" @click.prevent="cancelRegions" title="取消"><i class="pi pi-times"></i></a>
		</div>
		<div v-if="!isAdding && !isAddingBatch">
...
Vue.component("ns-access-log-box", {
	...
	<div v-if="accessLog.error != null && accessLog.error.length > 0" style="color:#dc143c">
		<i class="p-message-icon pi pi-exclamation-triangle"></i>错误：[{{accessLog.error}}]
	</div>
</div>`
})
